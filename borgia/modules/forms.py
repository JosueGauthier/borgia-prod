...
class ShopModuleSaleForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        if self.module_class == 'operator_sales':
            ...
            # :::
            if self.module.ask_password :
                self.fields['password'] = self.get_client_password()
    ...
    def clean(self):
        ...
        if self.client is None:
            try:
                ...
                # ::: 
                if self.module.ask_password:
                    if not check_password(self.cleaned_data['password'],self.client.password):
                        raise forms.ValidationError("Mot de passe incorect")
        ...
        for field in self.cleaned_data:
            #if field != 'client':
            # ::: 
            if field not in ['client', 'password'] :
...
class ShopModuleConfigForm(forms.Form):
    ...
    delay_post_purchase = forms.IntegerField(label="Durée (en secondes)",
                                             ...
                                             required=False)
    ask_password = forms.BooleanField(
            label="Demande du mot de passe lors d'un achat",
            required=False
    )
